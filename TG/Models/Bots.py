...
class Bot_Model(Model):
    COLUMNS = ['id', 'name', 'addresses', 'number', 'username', 'type', 'inns', 'status', 'balance', 'gender']
    table_name = 'bots'

    def __init__(self, id=0, name='', addresses=[], number='', username='', type='', inns=[], status='', balance='', gender=''):
        super().__init__()
        self.id = id
        self.name = name
